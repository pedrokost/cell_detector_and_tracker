...
fastApproximations = 1;  % avoid expensive computations and approximate quickly

stats = regionprops(object, 'Area','Orientation','PixelIdxList');

% clf; subplot(3,1,1); imagesc(object);axis equal; axis tight;
% Area is the fastest to compute from regionprops!!
if numel(stats) > 1
    maxIndx = 1;
    for i = 2:numel(stats)
    ...
    stats = stats(maxIndx);
    object = uint8(zeros(size(object)));
...
if fastApproximations
    angle = 0;
    stats = regionprops(object,'BoundingBox');
else
...
if numel(stats) > 1
    ...
    % subplot(3,1,2) ;imagesc(object); axis equal; axis tight;
    % stats = regionprops(object,'Centroid','PixelList','Area','PixelIdxList','BoundingBox');
    %Get the object Only inside the image
    if ~isempty(stats)
        ...
        stats = regionprops(object,'Centroid','PixelList');
        % subplot(3,1,3) ;imagesc(object); axis equal; axis tight;pause
    end
end


% imagesc(object); pause;
