function outstats = fastregionprops2(varargin)
...
if ~statsAlreadyComputed.PerimeterCornerPixelList
    ...
    for k = 1:length(stats)
        perimImage = fastbwmorph(stats(k).Image, 'perim8');
        firstRow = stats(k).BoundingBox(2) + 0.5;
        firstCol = stats(k).BoundingBox(1) + 0.5;
        [r,c] = find(perimImage);
...
%         startIdxForProp = 3;
%         reqStats = varargin(startIdxForProp:end);
...
% statsAlreadyComputed = struct('Area', 0, 'Centroid', 0, 'BoundingBox', 0, 'SubarrayIdx', 0, 'MajorAxisLength', 0, 'PerimeterCornerPixelList', 0, 'MinorAxisLength', 0, 'Eccentricity', 0, 'Orientation', 0, 'ConvexHull', 0, 'ConvexImage', 0, 'ConvexArea', 0, 'Image', 0, 'FilledImage', 0, 'FilledArea', 0, 'EulerNumber', 0, 'Extrema', 0, 'EquivDiameter', 0, 'Solidity', 0, 'Extent', 0, 'PixelIdxList', 0, 'PixelList', 0, 'Perimeter', 0, 'PerimeterOld', 0, 'PixelValues', 0, 'WeightedCentroid', 0, 'MeanIntensity', 0, 'MinIntensity', 0, 'MaxIntensity', 0);
