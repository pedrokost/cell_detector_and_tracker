...
trackletViewer(tracklets, 'in', struct('animate', false, 'showLabel', false));
title('Ground truth')
ax = axis(f1);

f1 = subplot(1,numGaps,2);
tracklets = generateTracklets('out', struct('withAnnotations', false));
trackletViewer(tracklets, 'out', struct('animate', false, 'showLabel', false));
...
for i=1:numel(maxGaps)
	...
	trackletViewer(tracklets, 'out', struct('animate', false));
	axis(f2, ax)
